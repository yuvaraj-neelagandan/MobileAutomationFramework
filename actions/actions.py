...
class Actions:
    ...
    def tap(self, locator, **kwargs):
        ...
        -> element(s)
        """
        Actions.is_displayed(self, locator, True)

        actions = TouchAction(self.driver)
        return {
            'element': lambda x: actions.tap(Actions.element(self, locator)).perform(),
            'elements': lambda x: actions.tap(Actions.elements(self, locator)[kwargs['index']]).perform()
    ...
    def double_tap(self, locator, n=3, **kwargs):
        ...
        -> element(s)
        """
        Actions.is_displayed(self, locator, True, n=n)

        actions = TouchAction(self.driver)
        return {
            'element': lambda x: actions.tap(Actions.element(self, locator), count=2).perform(),
            'elements': lambda x: actions.tap(Actions.elements(self, locator)[kwargs['index']], count=2).perform()
        }[keyword_check(kwargs)]('x')
    ...
    def click(self, locator, n=3, **kwargs):
        ...
        -> element(s)
        """
        Actions.sleep(kwargs)
        Actions.is_displayed(self, locator, True, n=n)

        return {
            'element': lambda x: Actions.element(self, locator).click(),
            'elements': lambda x: Actions.elements(self, locator)[kwargs['index']].click()
        }[keyword_check(kwargs)]('x')

    def wait_until_disActionsear(self, locator, n=3, **kwargs):
    ...
    def send_keys(self, locator, text='', **kwargs):
        ...
        return {
            'element': lambda text: Actions.element(self, locator).clear() and Actions.element(self, locator).send_keys(text),
            'elements': lambda text: Actions.elements(self, locator)[kwargs['index']].clear() and Actions.elements(self, locator)[kwargs['index']].send_keys(text)
    ...
    def close(self):
        self.driver.close_Actions()

    def reset(self):
        self.driver.reset()

    def launch_Actions(self):
        self.driver.launch_Actions()
    ...
    def swipe(self, start, dest):
        ...
        if type(start[1]) is not int:
            source_element = Actions.element(self, start)
        else:
            source_element = Actions.elements(self, start[0])[int(start[1])]

        if type(dest[1]) is not int:
            target_element = Actions.element(self, dest)
        else:
            target_element = Actions.elements(self, dest[0])[int(dest[1])]
    ...
    def assert_text(self, locator, text, n=20, **kwargs):
        ...
        while n > 1:
            try:
                if len(kwargs) == 0:
                    assert Actions.element(self, locator).text == text
                else:
                    assert Actions.elements(self, locator)[kwargs['index']].text == text
                break
            except Exception as e:
                ...
                n -= 1
                if len(kwargs) == 0:
                    if n == 1: assert Actions.element(self, locator).text == text
                else:
                    if n == 1: assert Actions.elements(self, locator)[kwargs['index']].text == text
    ...
    def assert_size(self, locator, param):
        ...
        maximum poll #20 with approx. ~10 secs
        """
        Actions.is_displayed(self, locator, True, index=0)

        case = param.rsplit(None, 1)[0]
        value = int(param.rsplit(None, 1)[1])

        if case in ['more than', 'greater than', 'above', '>']:
            assert Actions.elements(self, locator).__len__() > value
